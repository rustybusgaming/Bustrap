...
private void UpdateFilteredLibrary()
{
    ...
    Task.Run(() =>
    {
        ...
        /// <summary>
        /// Now playing in Spotify / TIDAL / Apple Music, read through Windows'
        /// media transport controls.
        /// </summary>
        public MediaSessionViewModel MediaSession { get; } = new();
        ...
        public double PositionSeconds
        {
            ...
            set
            {
                if (Math.Abs(_positionSeconds - value) > 0.25)
                {
                    ...
                    {
                        App.Logger.WriteException("MusicPlayerViewModel::RemoveTrack", ex);
                    }
        ...
        private void Stop()
        {
            ...
            {
                App.Logger.WriteException("MusicPlayerViewModel::Stop", error);
            }
        ...
        private void ConnectRpc(bool isAutoReconnect = false)
        {
            try
            {
                if (_rpcConnected && _rpcClient != null)
                {
                    ...
                    {
                        App.Logger.WriteException("MusicPlayerViewModel::ConnectRpc", error);
                    }
        ...
        private void UpdateRpcPresence(bool force = false)
        {
            ...
            if (NowPlaying == null || string.IsNullOrWhiteSpace(NowPlaying.Title))
            {
                ...
                {
                    Details = "Idle",
                    State = "Bustrap Music Player",
                    Assets = new DRPC.Assets
                    {
                        LargeImageKey = "Bustrap_logo",
                        LargeImageText = "Bustrap Music Player"
                    }
                };
            }
            else
            {
                ...
                string infoString = $"{(NowPlaying.FileType ?? "FILE").ToUpperInvariant()} • {elapsedStr} / {totalStr} • {(_isPlaying ? "Playing" : "Paused")} in Bustrap{loopText}";
            ...
            {
                App.Logger.WriteException("MusicPlayerViewModel::UpdateRpcPresence", error);
            }
        }
        ...
        private void TryProbeDuration(TrackItem item)
        {
            try
            {
                ...
                onOpened = (_, __) =>
                {
                    ...
                    finally
                    {
                        ...
                        try { probe.Close(); } catch (Exception exception)
                        {
                            App.Logger.WriteException("MusicPlayerViewModel::TryProbeDuration", exception);
                        }
                    }
                };
                probe.MediaOpened += onOpened;
            ...
            {
                App.Logger.WriteException("MusicPlayerViewModel::TryProbeDuration", error);
            }
        }
        ...
        private static ImageSource? GetFileIcon(string path)
        {
            try
            {
                ...
                {
                    try { NativeMethods.DeleteObject(hbitmap); } catch (Exception error)
                    {
                        App.Logger.WriteException("MusicPlayerViewModel::GetFileIcon", error);
                    }
                }
            }
            catch { return null; }
        ...
        public void Dispose()
        {
            try
            {
                ...
                SaveLibrary();
                try { _player.Stop(); } catch (Exception error)
                {
                    App.Logger.WriteException("MusicPlayerViewModel::Dispose", error);
                }
