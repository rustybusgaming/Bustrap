...
using static BustrapRobloxSettingsManager;


namespace Bustrap.UI.ViewModels.Settings
{
    public class BehaviourViewModel : NotifyPropertyChangedViewModel
    {
        ...
        {
            var settings = new BustrapRobloxSettings
            {
                MemoryCleanerIntervalSeconds =
                    SelectedMemoryCleanerInterval?.Seconds ?? 0
            };

            BustrapRobloxSettingsManager.Save(settings);
        }
        ...
        public bool CleanerBustrap
        {
            get => CleanerItems.Contains("BustrapLogs");
            set
            {
                if (value && !CleanerItems.Contains("BustrapLogs"))
                {
                    CleanerItems.Add("BustrapLogs");
                    UpdateCleanerItems();
                }
                else if (!value && CleanerItems.Contains("BustrapLogs"))
                {
                    CleanerItems.Remove("BustrapLogs");
                    UpdateCleanerItems();
                }
                OnPropertyChanged(nameof(CleanerBustrap));
            }
        }
